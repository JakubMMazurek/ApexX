...
The project includes an `apexxShowcase` Lightning Web Component and an `ApexX Showcase` Lightning tab. The tab calls focused `.clsx` methods such as `loadHotAccounts()`, `loadHotContactEmails()`, `loadAccountSummary()`, and `loadRevenueComparison()` so each section demonstrates a different ApexX feature: decorators, `filter`, `flatMap`, `map`, `count`, `all`, `find`, `Func`, and default-argument helpers. Seed demo data with:
